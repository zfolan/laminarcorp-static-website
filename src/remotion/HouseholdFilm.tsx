...
import { storyMoments } from '../data/storyMoments'
import { film } from './filmTokens'

const pageCenters = [0, 158, 281, 405, 528, 652]
const pageBounds = [0, 96, 219, 343, 466, 590, 713]
...
const layerMotion = (frame: number, index: number) => {
  ...
  if (index === 5) {
    return {
      opacity: interpolate(frame, [start - edge, start, end], [0, 1, 1], clamp),
      translateY: interpolate(frame, [start - edge, start, end], [38, 0, 0], { ...clamp, easing: Easing.inOut(Easing.cubic) }),
      scale: interpolate(frame, [start - edge, start, end], [.985, 1, 1], clamp),
      clip: interpolate(frame, [start - edge, start], [12, 0], clamp),
    }
  }
  return {
    opacity: interpolate(frame, [start - edge, start, end - edge, end], [0, 1, 1, 0], clamp),
    translateY: interpolate(frame, [start - edge, start, end - edge, end], [38, 0, 0, -22], { ...clamp, easing: Easing.inOut(Easing.cubic) }),
    scale: interpolate(frame, [start - edge, start, end - edge, end], [.985, 1, 1, .985], clamp),
    clip: interpolate(frame, [start - edge, start], [12, 0], clamp),
  }
}
...
const Screen = ({ frame, index, children }: { frame: number; index: number; children: ReactNode }) => {
  ...
  return (
    <section style={{ position: 'absolute', inset: '54px 0 0', overflow: 'hidden', opacity: motion.opacity, translate: `0 ${motion.translateY}px`, scale: motion.scale, clipPath: `inset(${motion.clip}% 0 0 0)`, transformOrigin: '50% 30%', pointerEvents: 'none' }}>
      {children}
    </section>
  )
}
...
const HouseholdScreen = ({ frame }: { frame: number }) => {
  const selected = interpolate(frame, [8, 18], [0, 1], { ...clamp, easing: Easing.out(Easing.cubic) })
  const lift = interpolate(frame, [10, 15, 19, 25], [0, 1, 1, 0], { ...clamp, easing: Easing.inOut(Easing.cubic) })
    ...
    <Screen frame={frame} index={0}>
      ...
      <div style={{ margin: '0 34px', border: `1px solid ${film.border}`, background: film.panel }}>
        ...
        {demoHouseholdQueue.map((household, index) => (
          <div key={household.name} style={{ position: 'relative', zIndex: index === 0 ? 2 : 1, isolation: index === 0 ? 'isolate' : undefined, display: 'grid', gridTemplateColumns: '1.2fr .5fr .65fr 1.05fr .45fr .55fr', minHeight: 63, padding: '0 16px', alignItems: 'center', borderBottom: `1px solid ${film.border}`, background: index === 0 ? `linear-gradient(90deg, rgba(84,135,182,${.05 + selected * .13}), rgba(84,135,182,${.015 + selected * .035}) 62%, ${film.panel})` : film.panel, transform: index === 0 ? `translateY(${-lift * 2.5}px) scale(${selectedScale})` : undefined, transformOrigin: '50% 50%', boxShadow: index === 0 ? `0 0 0 ${lift * 8}px ${film.panel}, 0 ${lift * 10}px ${lift * 28}px rgba(0,0,0,${lift * .34}), inset 0 0 0 1px rgba(84,135,182,${selected * .72})` : undefined }}>
...
const BarCompare = ({ current, target }: { current: number; target: number }) => (
  <div style={{ display: 'grid', gap: 6 }}>
    <div style={{ width: `${current}%`, height: 5, background: '#9aa6b2' }} />
    <div style={{ width: `${target}%`, height: 5, background: film.blue }} />
  </div>
)
  ...
  <Screen frame={frame} index={1}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1.42fr .72fr', gap: 12, margin: '12px 34px' }}>
      <div style={{ border: `1px solid ${film.border}`, background: film.panel }}>
        ...
        {demoAnalytics.drift.map((row) => <div key={row.label} style={{ display: 'grid', gridTemplateColumns: '.7fr 1.5fr .28fr .28fr .34fr', gap: 14, alignItems: 'center', padding: '14px 16px', borderBottom: `1px solid ${film.border}` }}><span style={{ fontSize: 11 }}>{row.label}</span><BarCompare current={row.current * 2} target={row.target * 2} /><span style={{ ...mono, fontSize: 10 }}>{row.current}%</span><span style={{ ...mono, color: film.blueBright, fontSize: 10 }}>{row.target}%</span><span style={{ ...mono, color: row.difference.startsWith('+') ? film.amber : film.red, fontSize: 10 }}>{row.difference}</span></div>)}
      </div>
      <div style={{ border: `1px solid ${film.border}`, background: film.panel, padding: 17 }}>
        ...
        <div style={{ display: 'flex', height: 8, marginTop: 22, overflow: 'hidden' }}>{demoAnalytics.currency.map((item, index) => <span key={item.label} style={{ width: `${item.value}%`, background: index === 0 ? '#9aa6b2' : film.blue }} />)}</div>
...
const TradeRows = ({ compact = false }: { compact?: boolean }) => (
  <div>
    ...
    {proposedTrades.map((trade) => <div key={`${trade.ticker}-${trade.account}`} style={{ display: 'grid', gridTemplateColumns: '.5fr .55fr 1fr .65fr 1.15fr .5fr', minHeight: compact ? 39 : 48, padding: '0 14px', alignItems: 'center', borderBottom: `1px solid ${film.border}`, background: trade.status === 'Review' ? `${film.amber}08` : 'transparent', ...mono, fontSize: 9 }}><span style={{ color: trade.side === 'Buy' ? film.green : film.red }}>{trade.side.toUpperCase()}</span><strong>{trade.ticker}</strong><span>{trade.account}</span><span>{trade.amount}</span><span style={{ color: film.muted }}>{trade.reason}</span><Status>{trade.status}</Status></div>)}
  </div>
)
  ...
  <Screen frame={frame} index={3}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1.15fr .85fr', gap: 12, margin: '18px 34px' }}>
      <div style={{ border: `1px solid ${film.border}`, background: film.panel }}>
        ...
        {exceptionQueue.map((item) => <div key={item.title} style={{ display: 'grid', gridTemplateColumns: '24px 1fr auto', alignItems: 'center', gap: 12, minHeight: 70, padding: '0 17px', borderBottom: `1px solid ${film.border}` }}><span style={{ width: 7, height: 7, borderRadius: '50%', background: item.tone === 'red' ? film.red : film.amber }} /><div><strong style={{ fontSize: 12, fontWeight: 500 }}>{item.title}</strong><div style={{ ...mono, color: film.muted, marginTop: 6, fontSize: 8 }}>{item.detail}</div></div><span style={{ ...mono, color: item.tone === 'red' ? film.red : film.amber, fontSize: 10 }}>{item.value}</span></div>)}
      </div>
      <div style={{ border: `1px solid ${film.strong}`, background: film.raised, padding: 23 }}>
        ...
        <div style={{ marginTop: 20, padding: '15px', textAlign: 'center', background: film.blue, color: '#071018', ...mono, fontSize: 10 }}>APPROVE REVIEWED TRADES →</div>
      </div>
    </div>
  </Screen>
)
  ...
  <Screen frame={frame} index={4}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '.78fr 1.22fr', gap: 12, margin: '18px 34px' }}>
      <div style={{ border: `1px solid ${film.border}`, background: film.panel }}>
        ...
        {demoDeployment.tranches.map((tranche, index) => <div key={tranche.name} style={{ padding: '18px 17px', borderBottom: `1px solid ${film.border}`, background: index === 0 ? `${film.blue}12` : 'transparent', boxShadow: index === 0 ? `inset 3px 0 ${film.blueBright}` : undefined }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><strong style={{ fontSize: 12, fontWeight: 500 }}>{tranche.name}</strong><Status>{tranche.status}</Status></div><div style={{ display: 'flex', gap: 18, marginTop: 9, ...mono, color: film.muted, fontSize: 8 }}><span>{String(tranche.orders).padStart(2, '0')} ORDERS</span><span>{tranche.currency}</span></div></div>)}
      </div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1.15fr .85fr', gap: 14, margin: '20px 34px' }}>
      <div style={{ border: `1px solid ${film.border}`, background: film.panel }}>
        ...
        <div style={{ padding: '17px' }}>
          {['Household and account context confirmed', 'Portfolio-manager review recorded', 'Held trades excluded from this tranche'].map((item) => <div key={item} style={{ display: 'flex', justifyContent: 'space-between', padding: '15px 0', borderBottom: `1px solid ${film.border}`, fontSize: 12 }}><span>{item}</span><span style={{ ...mono, color: film.green, fontSize: 9 }}>CONFIRMED ✓</span></div>)}
        </div>
      </div>
      <div style={{ border: `1px solid ${film.blue}88`, background: film.raised, padding: 25, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ marginTop: 'auto', padding: '17px', textAlign: 'center', background: film.blue, color: '#071018', ...mono, fontSize: 10 }}>EXPORT REVIEWED TRADES ↓</div>
      </div>
    </div>
  </Screen>
)
...
const NarrativeLayer = ({ frame, compact }: { frame: number; compact: boolean }) => (
  <div style={{ position: 'absolute', zIndex: 8, inset: 0, pointerEvents: 'none' }}>
    {storyMoments.map((moment, index) => {
      const motion = layerMotion(frame, index)
      return (
        <section key={moment.eyebrow} style={{ position: 'absolute', left: compact ? '6%' : '4.8%', right: compact ? '6%' : 'auto', top: compact ? '68%' : '23%', width: compact ? 'auto' : '22%', opacity: motion.opacity, translate: compact ? `0 ${motion.translateY * .45}px` : `${motion.translateY * -.25}px 0` }}>
          <div style={{ width: compact ? 54 : 74, height: 1, marginBottom: compact ? 15 : 25, background: film.blueBright }} />
          <div style={{ ...mono, color: film.blueBright, fontSize: compact ? 8 : 10, lineHeight: 1.2 }}>{moment.eyebrow}</div>
          <h2 style={{ margin: compact ? '11px 0 0' : '18px 0 0', maxWidth: compact ? 700 : 390, color: film.text, fontSize: compact ? 31 : 48, fontWeight: 500, lineHeight: .98, letterSpacing: '-.05em' }}>{moment.title}</h2>
          <p style={{ margin: compact ? '13px 0 0' : '25px 0 0', maxWidth: compact ? 720 : 360, color: film.muted, fontSize: compact ? 11 : 15, lineHeight: 1.52 }}>{moment.copy}</p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: compact ? 13 : 23, paddingTop: compact ? 11 : 15, borderTop: `1px solid ${film.border}` }}>
...
export const HouseholdFilm = () => {
  ...
  const rotateY = mix(frame, pageCenters, [.8, -.5, .4, -.45, .35, 0])
  const rotateX = mix(frame, pageCenters, [-.2, .14, -.11, .13, -.14, 0])
  const rotateZ = mix(frame, pageCenters, [-.04, .03, -.02, .025, -.018, 0])
  const scale = mix(frame, pageCenters, [.93, .98, 1, .985, 1.005, .94])
  const x = mix(frame, pageCenters, [26, -10, 8, -7, 10, 0])
  const y = mix(frame, pageCenters, [16, -8, 4, -9, 5, 10])
  const progress = interpolate(frame, [0, 713], [0, 100], clamp)
  const compact = width < 1300

  return (
    <AbsoluteFill style={{ overflow: 'hidden', background: film.canvas, color: film.text, fontFamily: 'Instrument Sans, sans-serif', perspective: 2200 }}>
      ...
      <div style={{ position: 'absolute', inset: compact ? '3.5% 3% 35% 3%' : '8% 2.5% 8% 29%', transformStyle: 'preserve-3d', transform: `translate3d(${x}px, ${y}px, 0) rotateX(${rotateX}deg) rotateY(${rotateY}deg) rotateZ(${rotateZ}deg) scale(${scale})`, transformOrigin: '50% 50%', filter: 'drop-shadow(0 55px 65px rgba(0,0,0,.6))' }}>
        <div style={{ position: 'absolute', inset: '14px -16px -15px 15px', transform: 'translateZ(-54px)', border: `1px solid ${film.border}`, background: '#05080c', boxShadow: '0 35px 80px rgba(0,0,0,.46)' }} />
        ...
        <div style={{ position: 'absolute', left: '2%', right: '2%', bottom: -18, height: 18, transform: 'rotateX(-80deg)', transformOrigin: 'top', background: '#05070a', border: `1px solid ${film.border}` }} />
        <div style={{ position: 'absolute', top: '2%', right: -18, bottom: '2%', width: 18, transform: 'rotateY(78deg)', transformOrigin: 'left', background: '#070a0f', border: `1px solid ${film.border}` }} />
      </div>
